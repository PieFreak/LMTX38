...
export default function GameOverview() {
  ...
  const startChallenge = async () => {
    try {
      const roundID = document.querySelector('input[name="challenge"]').value;

      const response = await axios.get(`http://localhost:5000/game/round/${roundID}`);
      console.log(response.data.questions);
      navigate('/gameplay', { state: { game: response.data.questions, isNewGame: false, roundID: roundID, creator: response.data } });
    } catch (error) {
    ...
    <div className="min-h-screen w-full bg-indigo-50">
      ...
      <div className="w-full bg-white mb-2 flex justify-center items-center flex-col">
        ...
        <div className="flex flex-col justify-center items-center">
          <p className="w-auto font-extrabold text-2xl md:text-3xl m-8 mb-6 text-center" >Fått en inbjudan? Skriv in kod nedan</p>
          <input
            name="challenge"
            className="mt-2 mb-4 border-2 rounded-sm text-lg text-center"
            placeholder=""
            style={{ height: "50px", width: '300px' }}
          />
          <button className="mb-8 border-2 border-b-4 border-r-slate-500 border-b-slate-400 bg-[#ffffff] px-4 hover:bg-[#abe9f03a] border-indigo-200 hover:shadow-xl shadow-2xl transition-all duration-500"
            onClick={e => {
              startChallenge();
            }}>
            <h2 className="font-extrabold text-center text-small md:text-base mx-2 my-1">Starta</h2>
          </button>

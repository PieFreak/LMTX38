...
const SettingPopup = ({ title, text, show, onClose }) => {
    ...
    return (
        <div onClick={() => { onClose(); setSelectedButton(10); }} className="bg-gray-800 bg-opacity-30 fixed flex justify-center items-center w-full h-full">
            <div onClick={(e) => { e.stopPropagation(); }} className="w-[16rem] border-indigo-200 border-2 border-b-4 border-r-slate-500 border-b-slate-400 md:mx-auto lg:mx-auto md:w-[36rem] lg:w-[40rem] max-w-3xl top-20 md:top-56 shadow-2xl shadow-black h-auto md:h-96 bg-gradient-to-b from-white via-[#f2edf1] to-indigo-50 fixed flex items-center justify-center">
                ...
                <div className="h-full">
                    ...
                    <div className="mx-auto flex flex-row items-center justify-center space-x-2 md:space-x-4 my-2">
                        <button className={`flex justify-center items-center rounded-md text-xs md:text-base border-2 w-10 h-6 md:h-8 md:w-12 border-gray-400 md:px-4 ${selectedButton === 5 ? 'bg-blue-200' : ' bg-white'}`} onClick={() => handleAmountOption(5)}>5</button>
                        <button className={`flex justify-center items-center rounded-md text-xs md:text-base border-2 w-10 h-6 md:h-8 md:w-12 border-gray-400 md:px-4 ${selectedButton === 10 ? 'bg-blue-200' : ' bg-white'}`} onClick={() => handleAmountOption(10)}>10</button>
                        <button className={`flex justify-center items-center rounded-md text-xs md:text-base border-2 w-10 h-6 md:h-8 md:w-12 border-gray-400 md:px-4 ${selectedButton === 20 ? 'bg-blue-200' : ' bg-white'}`} onClick={() => handleAmountOption(20)}>20</button>
                    </div>

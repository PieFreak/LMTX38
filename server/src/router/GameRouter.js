...
gameRouter.get("/round/:id", async (req, res) => {
    try {
        ...
        const user = req.session.user;
        if (user == null) {
            res.status(403).send(`Bad GET call to ${req.originalUrl} | access denied`);
            return;
        }
        if (typeof id !== "string") {
            res.status(400).send(`Bad GET call to ${req.originalUrl} | id should be a string, has type ${typeof id}`);
            return;
        }
...
gameRouter.post("/round/complete/", async (req, res) => {
   try {
       const {questions, roundID, score } = req.body;
       const user = req.session.user;
       if (user == null) {
           res.status(403).send(`Bad POST call to ${req.originalUrl} | access denied`);
           return;
       ...
       const round = await gameService.saveRoundChallenge(user.id, questions, roundID, score);
       if (round == null) {
           res.status(400).send(`Couldn't save the round, something went wrong!`);
           return;
       }
 ...
 * GET call for getting a round by its ID
 */
gameRouter.get("/round/info/:id", async (req, res) => {
    try {
        const { id } = req.params;
        const user = req.session.user;
        if (user == null) {
        ...
        const round = await gameService.getRoundInfo(id);
        if (round == null) {
            res.status(400).send(`Could not find a round with id ${id}!`);
            return;
        }
